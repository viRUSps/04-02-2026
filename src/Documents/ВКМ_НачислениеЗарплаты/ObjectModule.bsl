...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ЗапросДубли.УстановитьПараметр("ПериодРегистрации", НачалоМесяца(Дата));
	ЗапросДубли.УстановитьПараметр("СписокСотрудников", СписокСотрудников);
	ЗапросДубли.УстановитьПараметр("Ссылка", Ссылка);

	РезультатДубли = ЗапросДубли.Выполнить();

	Если Не РезультатДубли.Пустой() Тогда
		ВыборкаДубли = РезультатДубли.Выбрать();
		Пока ВыборкаДубли.Следующий() Цикл
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Ошибка! Сотруднику " + ВыборкаДубли.Сотрудник + 
			                  " уже начислена зарплата за этот месяц в документе " + ВыборкаДубли.Регистратор;
			Сообщение.Сообщить();
		КонецЦикла;
...
Процедура СформироватьДвиженияПоОкладу()
	...
	|	И ВКМ_НачислениеЗарплатыНачисления.ВидРасчета В (ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_ОсновныеНачисления.Оклад), ЗНАЧЕНИЕ(ПланВидовРасчета.ВКМ_ОсновныеНачисления.Отпускные))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ_ИнтервалыОклада.Оклад КАК РазмерОклада
	|ИЗ
	|	ВТ_ДанныеДокумента КАК ВТ_ДанныеДокумента
...
Процедура РасчитатьОклад()
	...
	|	ВКМ_ДанныеГрафика.РазмерОклада КАК РазмерОклада
	|ИЗ
...
Процедура СформироватьВзаиморасчеты()
    ...
    Для Каждого Запись Из Движения.ВКМ_ОсновныеНачисления Цикл
        Если Запись.Результат <> 0 Тогда
            Движение = Движения.ВКМ_ВзаиморасчетыССотрудниками.Добавить();
            Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
            Движение.Период      = Дата;
            Движение.Сотрудник   = Запись.Сотрудник;
            Движение.Сумма       = Запись.Результат;
        КонецЕсли;
    КонецЦикла;
    
    // Дополнительные начисления (Премии)
...
Процедура РасчитатьНДФЛ()
   ...
   // 1. Подготовка записей
    ...
    Для Каждого СтрИзОклада Из Движения.ВКМ_ОсновныеНачисления Цикл
        Движение = Движения.ВКМ_Удержания.Добавить();
        Движение.Сотрудник = СтрИзОклада.Сотрудник;
        Движение.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.НДФЛ;
        Движение.ПериодРегистрации = НачалоМесяца(Дата);
        
        Движение.БазовыйПериодНачало = СтрИзОклада.ПериодДействияНачало;
        Движение.БазовыйПериодКонец  = СтрИзОклада.ПериодДействияКонец;
    КонецЦикла;
    ...
    |	СУММА(ВТ_База.РезультатБаза) КАК ОбщаяБаза
    |ИЗ
    |	(ВЫБРАТЬ
    |		БазаОсновные.НомерСтроки КАК НомерСтроки,
    |		БазаОсновные.РезультатБаза КАК РезультатБаза
    |	ИЗ
    |		РегистрРасчета.ВКМ_Удержания.БазаВКМ_ОсновныеНачисления(&Измерения, &Измерения, , Регистратор = &Ссылка) КАК БазаОсновные
    |	
    |	ОБЪЕДИНИТЬ ВСЕ
    |	
    |	ВЫБРАТЬ
    |		БазаДоп.НомерСтроки,
    |		БазаДоп.РезультатБаза
    |	ИЗ
    |		РегистрРасчета.ВКМ_Удержания.БазаВКМ_ДополнительныеНачисления(&Измерения, &Измерения, , Регистратор = &Ссылка) КАК БазаДоп) КАК ВТ_База
    |
    |СГРУППИРОВАТЬ ПО
    |	ВТ_База.НомерСтроки";
    ...
    Если Не РезультатЗапроса.Пустой() Тогда
        ...
        Пока Выборка.Следующий() Цикл
            ...
            Если Индекс < Движения.ВКМ_Удержания.Количество() Тогда
                Движение = Движения.ВКМ_Удержания[Индекс];
                Движение.Результат = Выборка.ОбщаяБаза * 0.13;
            КонецЕсли;
        КонецЦикла;
    КонецЕсли;
