...
Функция  СоздатьАкты(Период, МассивДоговоров)  Экспорт  

	Период          = Период;
    СписокДоговоров = МассивДоговоров;
    ...
    ЗапросСуществующих.УстановитьПараметр("СписокДоговоров", СписокДоговоров);
    ...
    ВыборкаСуществующих = ЗапросСуществующих.Выполнить().Выбрать();
    
    СоответствиеСуществующих = Новый Соответствие;
    Пока ВыборкаСуществующих.Следующий() Цикл
        СоответствиеСуществующих.Вставить(ВыборкаСуществующих.Договор, ВыборкаСуществующих.Ссылка);
    КонецЦикла;

    Всего = СписокДоговоров.Количество();
    
    Для Счетчик = 0 По Всего - 1 Цикл
        
        ТекущийДоговор = СписокДоговоров[Счетчик];
        ...
        Процент = Цел(((Счетчик + 1) / Всего) * 100);
        ДлительныеОперации.СообщитьПрогресс(Процент, "Обработка договора: " + Строка(ТекущийДоговор));
